...
CFilterManager::ClearPair::ClearPair(bool active, const LColorf& value)
   : m_active(active),
     m_value(value)
   {
   // Empty
...
void CFilterManager::set_stacked_clears(DrawableRegion* region,
   ...
   for(int i = 0; i < GraphicsOutput::RTP_COUNT; ++i)
      ...
      const LColorf* value = &(clears0[i].m_value);
      if(!active)
         {
...
render_scene_into(const RenderSceneIntoParameters& params)
   ...
   quad.set_color(LColorf(1, 0.5, 0.5, 1));
