...
\end{center}
\caption{ \label{F_Typical_Assumptions_Etapa4} Cuadro de supuestos considerados. Detección, Navegación e Interacción}
\end{figure}
...
\end{center}
\caption{ \label{F_IPercept_Calibrate_Homography_Diagram} PerceptVideo: Homografía }
\end{figure}
...
Interacción Avanzada: Se añaden nuevos mecanismos de interacción que vaya más allá de la navegación. Ya no se usará sólo la posición del usuario, sino también información relativa a sus movimientos, de forma que pueda interactuar con los elementos existentes en la escena. Se éstudiará la Excentricidad de su cuerpo (si mantiene posturas compactas o con extremidades muy abiertas) y también se construirá una malla 3D de su cuerpo para detectar colisiones con los objetos del Mundo. La velocidad de la colisión determinará qué tipo de acción se realizará. Estas acciones afectarán a qué tipo de contenidos se crean en el mundo.
\\

Creación de contenidos: Hasta el momento se han usado escenas de prueba para comprobar el funcionamiento de la aplicación y sus capacidades. Sin embargo, la intención de la aplicación que se desea construir es que el usuario genere este entorno a partir de sus acciones. De esta forma, se habilitarán mecanismos para crear, modificar o destruir elementos del entorno. Además según su comportamiento se creará un perfil del Mundo, que llamaremos 'Psique' que servirá como punto principal para la composición. Afectará al audio y a los objetos que se crean. 
\\

Un punto de especial interés en la creación de contenidos, es que se desea crear elementos de vida artificial. No todos los objetos creados serán entidades pasivas, esperando la interacción del usuario. Algunos de ellos tendrán un comportamiento propio, siendo capaces de moverse, perseguir sus propios objetivos, influir en la creación de nuevos contenidos, reproducirse y evolucionar.
\\

Se incluye también en la creación de contenidos, el apartado de la composición musical. La composición procedimental es un campo de gran interés. Aunque no es el objetivo realizar un estudio profundo sobre la misma, se ha decidido añadir ciertas componentes. Se plantea el uso de una gramática para la composición. Basándose en la psique, que refleja el histórico de las acciones realizadas, se establece la estructura armónica de la composición, su ritmo y estilo. El conjunto de elementos creados en el entorno, también en base a la psique y en sus acciones, crearán el acompañamiento. Finalmente, las acciones inmediatas del usuario crearán la melodía.
\\

Con la finalización de esta fase, la aplicación ya se puede considerar como terminada, de cara a los objetivos que se querían conseguir, cerrando el ciclo de las técnicas que se deseaban explorar en el desarrollo del trabajo. Sin duda, existen diversas líneas de trabajo futuro muy interesantes. Por ejemplo, la posibilidad de conectar varios sistemas entre sí o, con la reciente aparición del dispositivo XBOX Kinect, sería interesante implementar un nuevo módulo de percepción que use esta tecnología, y comprobar su integración en el sistema.

\section{Consideraciones Previas}

Debido a motivos personales y profesionales el proyecto ha sufrido un paréntesis en el que el desarrollo se ha detenido. 
\\

Debido a esto y al reconocimiento de que posiblemente el alcance ideado inicialmente podía ser demasiado extenso, se han tomado decisiones para acotar la dimensión del mismo. De esta forma, se ha planteado simplificar tanto los mecanismos de interacción avanzada como de creación de contenidos, y las mejoras se añaden como posible trabajo futuro. Aunque la arquitectura del SDK soporta un número indeterminado de cámaras y displays, se relajará la exigencia y concentración sobre este apartado para la consecución de la propuesta de instalación, centrándonos en el uso de una única cámara.
\\

También ha sido necesario el cambio y actualización de algunas de las librerías de terceros utilizadas.
\\

Panda3D: Se actualiza a la versión 1.8.1, aporta la mayor ventaja de que han mejorado la estabilidad, corrigiendo algunos módulos que no eran thread-safe al compilar en Release.
...
A partir de ahora el usuario no sólo se moverá por el entorno, sino que interactuará con el mismo. Para ello, será necesario obtener, además de los datos generales como su posición absoluta o movimiento global, estudiar su cuerpo. El usuario tendrá un avatar, no necesariamente visible, dentro del entorno 3D que refleje sus acciones. 
\\

Aunque es realmente interesante el estudio del reconocimiento postural, se ha optado por usar un modelo más sencillo aunque suficientemente. Se usará la sustracción de fondo para obtener un modelo 3D del usuario. De forma adicional, se puede obtener la velocidad de movimiento de un punto concreto del avatar utilizando la detección de movimiento. Sin embargo, no habrá distinción explícita de miembros ni postura. Aunque se considera un apartado muy interesante para trabajo futuro. En caso de que se desee ampliar conocimiento a este respecto se recomiendan las lecturas BIBIITEMS.
\\

La interacción del usuario se puede clasificar en dos secciones:
\\

Interacción: crear, modificar y destruir Entidades. Dependiendo de la situación del entorno y del elemento con el que interactúe en cada caso, esto provocará efectos personalizados. Afectará a la parte de la Psique que aumentará las componentes de Bondad o Maldad.
\\

Clasificación del movimiento: A través de los momentos es muy fácil obtener un valor de excentricidad de la forma de su cuerpo, lo que indica si mantiene una pose muy compacta o con las extremidades extendidas. Afectará a las componentes de Calma o Energía.
...
Durante esta Etapa se trabajará en los módulos de Producción para añadir la capacidad de crear un modelo 3D a partir de la sustracción del usuario (Presencia) obtenida del módulo de Percepción. También se añadirá un esquema de colisiones Avatar-Entidades y, dado un punto de colisión sobre la malla del avatar obtener la velocidad de movimiento del jugador en ese segmento de su cuerpo.
\\

El propio motor de juego nos da herramientas para la detección de colisiones, y el módulo de percepción es capaz de darnos la segmentación del usuario y la velocidad en cada pixel de la imagen, como ya se ha descrito en los apartados anteriores. De esta forma, el apartado más interesante de esta sección es la contrucción de una malla 3D que represente al usuario, a partir de una nube de puntos. Para ello se ha usado el algoritmo de Marching Cubes que, dado un punto y una vecindad crea primitivas predefinidas (vértices, aristas y caras) o las descarta. Este problema es muy costoso en cómputo y muy sensible a la resolución, por ello requiere una simplificación. 
\\
...
Se añaden las secciones en la configuración de las cámaras para permitir usar imagen de cámara real o cargar un fichero de video. También se activa la capacidad de grabar las cámaras actuales, para poder reutilizar dicha secuencia.

\subsubsection{Diseño en UML - Módulo de Persistencia}
\subsubsection{Diseño en UML - Módulo de GUI}
\subsubsection{Diseño en UML - Módulo de Producción}

Modelo 3D del avatar con Marching Cubes.

Deconstrucción de la nube de puntos.
...
Dado que el uso será extensivo se añade la capacidad de que, en lugar de tomar la imagen de una cámara en vivo, el sistema se alimente con un fichero de video.
...
\bibitem{GREGMJM} Greg Mori, Jitendra Malik {\em  } Estimating Human Body configurations using Shape Context Matching oyo2 chacho hazme caso
